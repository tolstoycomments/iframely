...
const providers = JSON.parse(await readFile(pathToFileURL('./providers.json')));
 ...
 export function getOembed(uri, options, callback) {
    ...
    try {
        var params = ADD_OEMBED_PARAMS.find(params => {
            return Array.isArray(params.re) && params.re.find(re => re.test(uri)); // also checks that uri is defined
        });
        if (params) {
    ...
    if (contentType && contentType.match('xml')) {
        ...
        async.some([jsonStream2oembed, xmlStream2oembed], function(processor, cb) {
            processor(stream, function(error, data) {
                if (!error && data && Object.keys(data).length > 2) { // keys check - xmlStream2oembed creates empty {} json on opentag and returns it if any error
                                                                      // Also, when type is text/plain, but content is actually JSON, xml parser processes html and adds `iframe:` attribute, 
                                                                      // ignoring other fields. Ex.: https://twinmotion.unrealengine.com/panorama/Qh7JDUAbKupvvhVO
                    oembed = data;
                    cb(null, true);
                } else {
                    cb(null, false);
