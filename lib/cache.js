    ...
    function getCustomCacheOverride(key) {
        var override = CONFIG.CACHE_KEY_OVERRIDES && CONFIG.CACHE_KEY_OVERRIDES.find(item => {
            return item && item.re && item.re.some(re => re.test(key));
        });
        return override;
    }

    function prepareKey(key) {
        var override = getCustomCacheOverride(key);
        var new_key = override && override.replace && override.replace(key) || key;
        return {
            replaced_key: new_key !== key && new_key,
            retry_get: override && override.retry_get,
            retry_set: override && override.retry_set,
        };
    }
    ...
    async function setCachingEngine(path) {
        try {
            ...
            var importPath = path.match(/^[a-zA-Z]:/) ? pathToFileURL(path).href : path;
            var cache_engine = await import(importPath);
            ...
            cache.set = function(key, data, options) {
                ...
                if (custom_ttl !== 0) {
                    ...
                    // Retry set with replaced key.
                    var preparedKey = prepareKey(key);
                    if (preparedKey.retry_set && preparedKey.replaced_key) {
                        cache_engine.set(preparedKey.replaced_key, data, options);
                    }
                }
            };
            // TODO: do not get if ttl === 0 ?
            cache.get = function(key, cb) {
                cache_engine.get(key, function(error, data) {
                    var preparedKey = prepareKey(key);
                    if (!error 
                        && !data 
                        && preparedKey.replaced_key 
                        && preparedKey.retry_get) {

                            // Retry with replaced key.
                            cache_engine.get(preparedKey.replaced_key, cb);
